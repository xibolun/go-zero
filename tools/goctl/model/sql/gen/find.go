...
func genFind(table Table, withCache, postgreSql bool) (string, error) {
	camel := table.Name.ToCamel()
	text, err := pathx.LoadTemplate(category, findTemplateFile, template.Find)
	if err != nil {
		return "", err
	}

	output, err := util.With("find").
		Parse(text).
